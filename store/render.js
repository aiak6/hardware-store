...
const { GLOSSARY, CLUSTER_TRUTH, CONVERSIONS } = require("./data");
...
<header class="site">
  ...
  <div class="header-btns">
    <button id="sound-toggle" class="theme-toggle" type="button" aria-label="Toggle interface sounds" title="Sounds on / off">🔊</button>
...
function home() {
  ...
  <section class="hero">
    ...
    <div class="cta-row rise" style="animation-delay:.85s">
      ...
      <a class="btn" href="/advisor">Help me choose</a>
      <a class="btn" href="/compare">Compare hardware</a>
      <a class="btn ghost" href="/products">Walk the aisles — $2K to $3.5M</a>
    </div>
  </section>
...
function products(list) {
  const card = (p) => `
    <article class="card product" id="${p.id}">
      ...
      <div class="stats">
        ...
        ${stat("Power draw", num(p.watts) + " W", "watts")}
        ${stat("Memory type", esc(p.memoryType), "gpuMemory", "This is the kind and amount of the GPU's own memory — the pool your model's weights must fit inside.")}
  ...
  const aisleHtml = aisles.map((a, i) => `
    <h2 class="aisle"><span class="aisle-num">Aisle ${i + 1}</span> ${esc(a.tier)}${a.items.length > 1 ? "s" : ""}</h2>
  ...
  const embed = JSON.stringify({
    ...
    modelsMini: [
      { name: "Qwen3-235B", params: 235 },
      { name: "DeepSeek-V3 (671B)", params: 671 },
      { name: "Kimi K2 (1000B)", params: 1000 },
    ],
  }).replace(/</g, "\\u003c");
...
// ---- Quote form -----------------------------------------------------------
function quoteForm(model, rec, typeLabel) {
  const summary = rec
  ...
  <section class="card form-card">
    ...
    <form method="POST" action="/quote">
      ...
      <label>Your name
        <input type="text" name="customer_name" required placeholder="Jane Doe">
      </label>
      <label>Contact (email or phone)
        <input type="text" name="contact" required placeholder="jane@company.com">
      </label>
      <button class="btn primary" type="submit">Submit request →</button>
    </form>
  </section>`;
  return layout("Request a quote", body, "/advisor");
}
  ...
  const items = rows.length
    ? rows
      ...
      <tr>
        ...
        <td>${esc(r.build_summary || "—")}</td>
        <td class="mono">${r.total_price ? money(r.total_price) : "—"}</td>
